 ...
 * Practically speaking, this means that Arlequin will always incorrectly assign statistical significance to any SNP with an FST = 1 that has evolved under these conditions. Because observed at least one SNP with an FST = 1 in each of the 100 replicate datasets, Arlequin incorrectly inferred statistical significance in 100% of these datasets. 
 * With Gppfst, only ~4% (4/100) of the simulated datasets yield an excess of SNPs with an FST = 1 that incorrectly inferred to be statistically significant
 * The main difference between Arlequin and Gppfst is that Arlequin computes significance for individual loci conditioned on the observed level of heterozygosities, whereas GppFst will compute the probability of observing an empirical proportion of loci within a given Fst range conditioned on the particular coalescent model of population divergence (see Fig. 1 in manuscript and plots)
 * To summarize, with a genomic-sized dataset of >1,000 unlinked neutral SNPs, we expect a certain proportion of loci to be completely fixed in different populations (FST = 1) that are not at equilibrium. GppFst – unlike many other methods – will more appropriately account for this expectation. 
