...
We compared our method to the Arlequin function for detecting loci under selection. The results of this analysis are provided in the vignettes folder, including the Arelquin output file (Example_ArlequinResults.csv), and two plots showing the joint distribution of heterozygosity and significance (Example_lociSelection_FST.png,Example_lociSelection_Names_FST.png). There are a number of main points are clear from these comparisons:
 * There is a total of 1691 loci that are statistically significant with Arlequin; these 1691 loci are distributed across the Fst spectrum (see plots). However, the bulk of statistically significant loci are found with Fst < 0.
 * Arlequin could not assign statistical significance to loci with FST of 1, whereas GppFst will compute the probability of observing a proportion of loci wth FST of 1 (see Fig. 1 in manuscript)
