 ...
 * With Arlequin, 100% of SNPs with an FST = 1 were statistically significant in 100% (100/100) of the simulated datasets (you can see these results in the individual Arlequin output files that are located in Example_ArlequinResults.zip) – Each of the 100 simulated datasets have at least one SNP with an FST = 1, which is always incorrectly inferred to be statistically significant with Arlequin.
 * Practically speaking, this means that every time Arlequin is run on a set of neutral loci that evolved under these conditions, Arlequin will always assign statistical significance to any SNP with an FST = 1. 
 ...
 * The main difference between Arlequin and Gppfst is that Arlequin computes significance for individual loci conditioned on the observed level of heterozygosities, whereas GppFst will compute the probability of observing an empirical proportion of loci within a given Fst range conditioned on the particular coalescent model of population divergence (see Fig. 1 in manuscript and plots)
 * To summarize, we expect a certain proportion of loci that evolved under strictly neutral conditions to be completely fixed in different populations (FST = 1) that are not at equilibrium. GppFst – unlike many other methods – will more appropriately account for this expectation. 
